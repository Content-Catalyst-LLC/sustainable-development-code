from __future__ import annotations

import pandas as pd
import numpy as np

INPUT_FILE = "digital_infrastructure_capacity_panel.csv"
OUTPUT_FILE = "dpi_readiness_and_service_capacity_scores.csv"


def load_data(path: str) -> pd.DataFrame:
    """Load digital infrastructure and institutional capacity data."""
    df = pd.read_csv(path)

    required_columns = [
        "country",
        "region",
        "sector",
        "connectivity_index",
        "digital_identity_index",
        "payments_rail_index",
        "data_exchange_index",
        "registry_integrity_index",
        "service_delivery_index",
        "cybersecurity_index",
        "public_trust_index",
        "inclusion_access_index",
        "compute_cloud_index",
        "interoperability_index",
        "institutional_use_capacity_index",
        "lock_in_risk_index",
    ]

    missing = [col for col in required_columns if col not in df.columns]
    if missing:
        raise ValueError(f"Missing required columns: {missing}")

    return df


def validate_indices(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure all *_index columns are bounded in [0, 1]."""
    index_columns = [col for col in df.columns if col.endswith("_index")]
    for col in index_columns:
        if ((df[col] < 0) | (df[col] > 1)).any():
            raise ValueError(f"Column '{col}' contains values outside [0, 1].")
    return df


def compute_scores(df: pd.DataFrame) -> pd.DataFrame:
    """
    Compute:
    - foundational digital infrastructure score
    - digital public infrastructure score
    - service-capacity score
    - constrained development-capacity score
    """
    df = df.copy()

    df["foundational_digital_infrastructure_score"] = (
        0.25 * df["connectivity_index"] +
        0.15 * df["compute_cloud_index"] +
        0.15 * df["cybersecurity_index"] +
        0.15 * df["registry_integrity_index"] +
        0.15 * df["interoperability_index"] +
        0.15 * df["inclusion_access_index"]
    ).clip(lower=0, upper=1)

    df["digital_public_infrastructure_score"] = (
        0.20 * df["digital_identity_index"] +
        0.20 * df["payments_rail_index"] +
        0.20 * df["data_exchange_index"] +
        0.15 * df["registry_integrity_index"] +
        0.15 * df["interoperability_index"] +
        0.10 * df["cybersecurity_index"]
    ).clip(lower=0, upper=1)

    df["service_capacity_score"] = (
        0.30 * df["service_delivery_index"] +
        0.20 * df["institutional_use_capacity_index"] +
        0.15 * df["public_trust_index"] +
        0.15 * df["inclusion_access_index"] +
        0.10 * df["digital_public_infrastructure_score"] +
        0.10 * df["foundational_digital_infrastructure_score"]
    ).clip(lower=0, upper=1)

    df["constrained_digital_development_capacity_score"] = (
        0.35 * df["foundational_digital_infrastructure_score"] +
        0.30 * df["digital_public_infrastructure_score"] +
        0.25 * df["service_capacity_score"] +
        0.10 * (1 - df["lock_in_risk_index"])
    ).clip(lower=0, upper=1)

    df["capacity_band"] = np.select(
        [
            df["constrained_digital_development_capacity_score"] >= 0.80,
            df["constrained_digital_development_capacity_score"] >= 0.60,
            df["constrained_digital_development_capacity_score"] >= 0.40,
        ],
        [
            "High digital capacity",
            "Strong digital capacity",
            "Moderate digital capacity",
        ],
        default="Constrained digital capacity",
    )

    return df


def build_summary(df: pd.DataFrame) -> pd.DataFrame:
    cols = [
        "country",
        "region",
        "sector",
        "foundational_digital_infrastructure_score",
        "digital_public_infrastructure_score",
        "service_capacity_score",
        "constrained_digital_development_capacity_score",
        "capacity_band",
    ]
    summary = df[cols].copy()
    summary = summary.sort_values(
        by=[
            "constrained_digital_development_capacity_score",
            "digital_public_infrastructure_score",
            "service_capacity_score",
        ],
        ascending=[False, False, False],
    )
    return summary


def main() -> None:
    df = load_data(INPUT_FILE)
    df = validate_indices(df)
    scored = compute_scores(df)
    summary = build_summary(scored)

    summary.to_csv(OUTPUT_FILE, index=False)

    print("DPI readiness and service-capacity scoring complete.")
    print(summary.to_string(index=False))


if __name__ == "__main__":
    main()
