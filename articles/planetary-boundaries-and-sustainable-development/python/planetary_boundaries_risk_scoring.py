...
def compute_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["earth_system_stress_score"] = (
        ...
        0.16 * df["freshwater_change_index"] +
        0.16 * df["land_system_change_index"] +
        0.16 * df["biogeochemical_pressure_index"] +
        0.16 * df["novel_entities_burden_index"]
    ).clip(lower=0, upper=1)

    df["development_exposure_score"] = (
        0.45 * df["justice_exposure_index"] +
        0.30 * df["biosphere_integrity_loss_index"] +
    ...
    df["constrained_planetary_development_score"] = (
        0.42 * df["earth_system_stress_score"] +
